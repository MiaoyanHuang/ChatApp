...
import com.example.fyp_chatapp.adapters.AddFriendAdapter;
import com.example.fyp_chatapp.databinding.ActivityAddFriendBinding;
...
public class AddFriendActivity extends BaseActivity implements AddFriendListener{
        ...
        database.collection(Constants.KEY_COLLECTION_USERS)
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult() != null && task.getResult().size() > 0) {
                        User user = new User();
                        user.id = task.getResult().getDocuments().get(0).getId();
                        user.name = task.getResult().getDocuments().get(0).getString(Constants.KEY_NAME);
                        user.email = task.getResult().getDocuments().get(0).getString(Constants.KEY_EMAIL);
                        user.image = task.getResult().getDocuments().get(0).getString(Constants.KEY_IMAGE);
                        user.token = task.getResult().getDocuments().get(0).getString(Constants.KEY_FCM_TOKEN);
    ...
    private void querySentRequest(User user) {
        database.collection(Constants.KEY_COLLECTION_REQUEST_ADD_FRIEND)
                ...
                .whereEqualTo(Constants.KEY_REQUEST_ADD_REQUEST_TO_EMAIL, user.email)
    ...
    public void onAddClicked(User user) {
        ...
        request.put(Constants.KEY_REQUEST_ADD_REQUEST_TO_ID, user.id);
        request.put(Constants.KEY_REQUEST_ADD_REQUEST_TO_NAME, user.name);
        request.put(Constants.KEY_REQUEST_ADD_REQUEST_TO_EMAIL, user.email);
        request.put(Constants.KEY_REQUEST_ADD_REQUEST_TO_IMAGE, user.image);
