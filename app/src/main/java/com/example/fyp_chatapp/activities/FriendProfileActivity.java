...
import com.example.fyp_chatapp.databinding.ActivityFriendProfileBinding;
...
public class FriendProfileActivity extends BaseActivity {
    ...
    private void removeFriend() {
        ...
        database.collection(Constants.KEY_COLLECTION_USERS)
                ...
                .update(Constants.KEY_FRIENDS, FieldValue.arrayRemove(user.id));
        ...
        //删除聊天记录
        database.collection(Constants.KEY_COLLECTION_CHAT).whereEqualTo(Constants.KEY_SENDER_ID, user.id)
                .whereEqualTo(Constants.KEY_RECEIVER_ID, preferenceManager.getString(Constants.KEY_USER_ID))
                .get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                    ...
                    showMessage(user.name + " has been removed");
                    setResult(RESULT_OK);
                    finish();
                });
    }
    ...
    private void saveBackgroundImageToTempDir(Bitmap bitmap) { //设置聊天背景
        ...
        String friendId = user.id; // 获取好友的id
