...
import com.hmy.chatapp.adapters.FriendRequestAdapter;
import com.hmy.chatapp.databinding.ActivityFriendRequestBinding;
...
public class FriendRequestActivity extends BaseActivity implements FriendRequestListener {
                ...
                .addOnCompleteListener(task -> {
                    ...
                    if(task.isSuccessful() && task.getResult() != null){
                        ...
                        for(QueryDocumentSnapshot documentSnapshot : task.getResult()){
                            ...
                            friendRequest.setDocumentId(documentSnapshot.getId());
                            friendRequest.setRequestFromId(documentSnapshot.getString(Constants.KEY_REQUEST_ADD_REQUEST_FROM_ID));
                            friendRequest.setRequestFromName(documentSnapshot.getString(Constants.KEY_REQUEST_ADD_REQUEST_FROM_NAME));
                            friendRequest.setRequestFromEmail(documentSnapshot.getString(Constants.KEY_REQUEST_ADD_REQUEST_FROM_EMAIL));
                            friendRequest.setRequestFromImage(documentSnapshot.getString(Constants.KEY_REQUEST_ADD_REQUEST_FROM_IMAGE));
    ...
    private void addFriend(FriendRequest friendRequest){
        database.collection(Constants.KEY_COLLECTION_USERS)
                ...
                .update("friends", FieldValue.arrayUnion(friendRequest.getRequestFromId()))
                .addOnSuccessListener(task -> database.collection(Constants.KEY_COLLECTION_USERS)
                        .document(friendRequest.getRequestFromId())
    ...
    private void updateRequest(boolean isAccepted, FriendRequest friendRequest) {
        database.collection(Constants.KEY_COLLECTION_REQUEST_ADD_FRIEND)
                .document(friendRequest.getDocumentId())
